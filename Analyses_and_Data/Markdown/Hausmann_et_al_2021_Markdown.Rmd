...
pref_stat_TLC_HC$light_HC_corrected[check_for_blue]<-pref_stat_TLC_HC$light_HC_corrected[check_for_blue]*0.85
...
```{r}
difference_posterior_slopes<-group_posteriors_main_interact$value[group_posteriors_main_interact$type=="HC"]-group_posteriors_main_interact$value[group_posteriors_main_interact$type=="TLC"]
```
...
print(data.frame(test=c("HC > TLC","HC > 0","TLC < 0"),
                 posterior_prob=c(sum(difference_posterior_slopes>0)/iterations,sum(group_posteriors_main_interact$value[group_posteriors_main_interact$type=="HC"]>0)/iterations,
...
```{r}
TLC_posterior_cat_1<-group_posteriors_cat$value[group_posteriors_cat$type=="TLC"&group_posteriors_cat$quantile_low_up_1=="q1"]
HC_posterior_cat_1<-group_posteriors_cat$value[group_posteriors_cat$type=="HC"&group_posteriors_cat$quantile_low_up_1=="q1"]
TLC_posterior_cat_2<-group_posteriors_cat$value[group_posteriors_cat$type=="TLC"&group_posteriors_cat$quantile_low_up_1=="q2"]
HC_posterior_cat_2<-group_posteriors_cat$value[group_posteriors_cat$type=="HC"&group_posteriors_cat$quantile_low_up_1=="q2"]
```
...
group_posteriors_three_way_HC<-extract_emm_posterior(group_est_three_way_HC)
iterations<-nrow(group_posteriors_three_way_HC)/2
print(data.frame(test=c("HC > TLC","HC > 0","TLC < 0"),
                 posterior_prob=c(sum(group_posteriors_three_way_HC$value[group_posteriors_three_way_HC$type=="HC"]>group_posteriors_three_way_HC$value[group_posteriors_three_way_HC$type=="TLC"])/iterations,sum(group_posteriors_three_way_HC$value[group_posteriors_three_way_HC$type=="HC"]>0)/iterations,
                                  sum(group_posteriors_three_way_HC$value[group_posteriors_three_way_HC$type=="TLC"]<0)/iterations)))
```
...
group_posteriors_three_way_TLC<-extract_emm_posterior(group_est_three_way_TLC)
iterations<-nrow(group_posteriors_three_way_TLC)/2
print(data.frame(test=c("HC > TLC","HC > 0","TLC < 0"),
                 posterior_prob=c(sum(group_posteriors_three_way_TLC$value[group_posteriors_three_way_TLC$type=="HC"]>group_posteriors_three_way_TLC$value[group_posteriors_three_way_TLC$type=="TLC"])/iterations,sum(group_posteriors_three_way_TLC$value[group_posteriors_three_way_TLC$type=="HC"]>0)/iterations,
                                  sum(group_posteriors_three_way_TLC$value[group_posteriors_three_way_TLC$type=="TLC"]<0)/iterations)))
```
...
```{r}
group_est_main_interact_corrected<-suppressWarnings(suppressMessages(emtrends(mod1.208_corrected_final, ~ type, var="log_light_HC_corrected",transform="response")))
```
...
We get the posteriors for the simple slopes
```{r}
group_posteriors_main_interact_corrected<-extract_emm_posterior(group_est_main_interact_corrected)
```

We get a difference posterior for the slopes.
```{r}
difference_posterior_slopes_corrected<-group_posteriors_main_interact_corrected$value[group_posteriors_main_interact_corrected$type=="HC"]-group_posteriors_main_interact_corrected$value[group_posteriors_main_interact_corrected$type=="TLC"]
```
...
3) Increase in log-illuminance around the *heu* 'female' leads to an increase in preference of *lin* males for their own type (negative slope).
```{r}
iterations<-nrow(group_posteriors_main_interact_corrected)/2
print(data.frame(test=c("HC > TLC","HC > 0","TLC < 0"),
                 posterior_prob=c(sum(difference_posterior_slopes_corrected>0)/iterations,sum(group_posteriors_main_interact_corrected$value[group_posteriors_main_interact_corrected$type=="HC"]>0)/iterations,
                                  sum(group_posteriors_main_interact_corrected$value[group_posteriors_main_interact_corrected$type=="TLC"]<0)/iterations)))
```
...
print(data.frame(test=c("HC (corrected) > HC (uncorrected)","TLC (corrected) < TLC (uncorrected)","HC-TLC (corrected) > HC-TLC (uncorrected)"),
                 posterior_prob=c(sum(group_posteriors_main_interact_corrected$value[group_posteriors_main_interact_corrected$type=="HC"]>mean(group_posteriors_main_interact$value[group_posteriors_main_interact$type=="HC"]))/iterations,sum(group_posteriors_main_interact_corrected$value[group_posteriors_main_interact_corrected$type=="TLC"]<mean(group_posteriors_main_interact$value[group_posteriors_main_interact$type=="TLC"]))/iterations,sum(difference_posterior_slopes_corrected>mean(difference_posterior_slopes))/iterations)))
```
...
We extract the posteriors.
```{r}
group_posteriors_light_class<-extract_emm_posterior(group_est_light_class)
```

Split posteriors by type and illuminance
```{r}
TLC_posterior_light_class_1<-group_posteriors_light_class$value[group_posteriors_light_class$type=="TLC"&group_posteriors_light_class$light_class_HC=="sunny"]
HC_posterior_light_class_1<-group_posteriors_light_class$value[group_posteriors_light_class$type=="HC"&group_posteriors_light_class$light_class_HC=="sunny"]
TLC_posterior_light_class_2<-group_posteriors_light_class$value[group_posteriors_light_class$type=="TLC"&group_posteriors_light_class$light_class_HC=="cloudy"]
HC_posterior_light_class_2<-group_posteriors_light_class$value[group_posteriors_light_class$type=="HC"&group_posteriors_light_class$light_class_HC=="cloudy"]
TLC_posterior_light_class_3<-group_posteriors_light_class$value[group_posteriors_light_class$type=="TLC"&group_posteriors_light_class$light_class_HC=="sunny_but_treeshade"]
HC_posterior_light_class_3<-group_posteriors_light_class$value[group_posteriors_light_class$type=="HC"&group_posteriors_light_class$light_class_HC=="sunny_but_treeshade"]
```

Create "difference posteriors"
```{r}
difference_posterior_light_class_1<-HC_posterior_light_class_1-TLC_posterior_light_class_1
difference_posterior_light_class_2<-HC_posterior_light_class_2-TLC_posterior_light_class_2
difference_posterior_light_class_3<-HC_posterior_light_class_3-TLC_posterior_light_class_3
```

Now we take a look at mean and 95% CrI for *lin*, *heu* and the difference between them.
```{r}
est_light_class<-data.frame(rep(c("TLC","HC","HC-TLC"),3),rep(c("sunny","cloudy","woodland shade"),each=3),rbind(c(mean(TLC_posterior_light_class_1),quantile(TLC_posterior_light_class_1,probs=c(0.025,0.975))),c(mean(HC_posterior_light_class_1),quantile(HC_posterior_light_class_1,probs=c(0.025,0.975))),c(mean(difference_posterior_light_class_1),quantile(difference_posterior_light_class_1,probs=c(0.025,0.975))),c(mean(TLC_posterior_light_class_2),quantile(TLC_posterior_light_class_2,probs=c(0.025,0.975))),c(mean(HC_posterior_light_class_2),quantile(HC_posterior_light_class_2,probs=c(0.025,0.975))),c(mean(difference_posterior_light_class_2),quantile(difference_posterior_light_class_2,probs=c(0.025,0.975))),c(mean(TLC_posterior_light_class_3),quantile(TLC_posterior_light_class_3,probs=c(0.025,0.975))),c(mean(HC_posterior_light_class_3),quantile(HC_posterior_light_class_3,probs=c(0.025,0.975))),c(mean(difference_posterior_light_class_3),quantile(difference_posterior_light_class_3,probs=c(0.025,0.975)))))
...
```{r}
non_red_posterior_cat_1<-group_posteriors_cat$value[group_posteriors_cat$redYN=="N"&group_posteriors_cat$quantile_low_up_1=="q1"]
red_posterior_cat_1<-group_posteriors_cat$value[group_posteriors_cat$redYN=="Y"&group_posteriors_cat$quantile_low_up_1=="q1"]
non_red_posterior_cat_2<-group_posteriors_cat$value[group_posteriors_cat$redYN=="N"&group_posteriors_cat$quantile_low_up_1=="q2"]
red_posterior_cat_2<-group_posteriors_cat$value[group_posteriors_cat$redYN=="Y"&group_posteriors_cat$quantile_low_up_1=="q2"]
```

Create "difference posteriors"
```{r}
difference_posterior_BC_cat_1<-red_posterior_cat_1-non_red_posterior_cat_1
difference_posterior_BC_cat_2<-red_posterior_cat_2-non_red_posterior_cat_2
```
...
names(est_types)<-c("object","light_env_heu","mean","lower","upper")
contrast_table<-unname(data.frame(rep("Y-N",2),c("q1","q2"),rbind(c(mean(difference_posterior_BC_cat_1),quantile(difference_posterior_BC_cat_1,probs=c(0.025,0.975))),c(mean(difference_posterior_BC_cat_2),quantile(difference_posterior_BC_cat_2,probs=c(0.025,0.975)))),stringsAsFactors=F))
...
testout_bayes<-plot_proportion_stats(input_data=pref_stat_TLC_HC[ifelse(is.na(pref_stat_TLC_HC$quantile_low_up_1),F,pref_stat_TLC_HC$quantile_low_up_1=="q1"),],
                                     # The data set
                                     sum_by_col_plot=c("ID"),
                                     # Sum by ID for plot
                                     response_col="court_lin_heu_01",
...
testout_bayes<-plot_proportion_stats(input_data=pref_stat_TLC_HC[ifelse(is.na(pref_stat_TLC_HC$quantile_low_up_1),F,pref_stat_TLC_HC$quantile_low_up_1=="q2"),],
                                     # The data set
                                     sum_by_col_plot=c("ID"),
                                     # Sum by ID for plot
                                     response_col="court_lin_heu_01",
...
```{r}
post_hist_class_1<-hist(difference_posterior_light_class_1,breaks=seq(floor(200*min(difference_posterior_light_class_1))/200,
                                                          ceiling(200*max(difference_posterior_light_class_1))/200,0.005),plot=F)
post_hist_class_2<-hist(difference_posterior_light_class_2,breaks=seq(floor(200*min(difference_posterior_light_class_2))/200,
                                                            ceiling(200*max(difference_posterior_light_class_2))/200,0.005),plot=F)
post_hist_class_3<-hist(difference_posterior_light_class_3,breaks=seq(floor(200*min(difference_posterior_light_class_3))/200,
                                                               ceiling(200*max(difference_posterior_light_class_3))/200,0.005),plot=F)
```
...
par(mar=c(2,0,2,0))
beeswarm(pref_stat_TLC_HC$log_light_HC~factor(pref_stat_TLC_HC$light_class_HC,levels=c("sunny","cloudy","sunny_but_treeshade")),
         cex=0.55,xaxt="n",yaxt="n",xlab="",ylab="",pch=21,xaxs="i")
#Add corrected measurements next to the woodland shade category
beeswarm(pref_stat_TLC_HC$log_light_HC_corrected[pref_stat_TLC_HC$light_class_HC=="sunny_but_treeshade"],
         cex=0.55,xaxt="n",yaxt="n",xlab="",ylab="",pch=21,at=3.18,col=adjustcolor("lightblue",0.7),add=T)
#Axes and title
axis(1,at=c(1,1.5,2,3),c("sunny","'open/cloudy'","cloudy","woodland shade"),lwd=0,line=0.2,cex.axis=2.5)
par(xpd=NA)
...
#We add the same type of swarm plot as before, but this time just including data from the sun-exposed heu model light class
testout_bayes<-plot_proportion_stats(input_data=pref_stat_TLC_HC[ifelse(is.na(pref_stat_TLC_HC$light_class_HC),F,pref_stat_TLC_HC$light_class_HC=="sunny"),],
                                     # The data set
                                     sum_by_col_plot=c("ID"),
                                     # Sum by ID for plot
                                     response_col="court_lin_heu_01",
...
#Scale all histograms so that length of the maximum bar of all posteriors is maximally 7/9 of the distance the axis has to the plot
max_bar<-max(c(post_hist_class_1$counts,post_hist_class_2$counts,post_hist_class_3$counts))
post_hist_class_1$counts<-(post_hist_class_1$counts/max_bar)*(par("cex")*1.4*par("cin")[2]/par("pin")[1])*diff(par("usr")[1:2])
post_hist_class_2$counts<-(post_hist_class_2$counts/max_bar)*(par("cex")*1.4*par("cin")[2]/par("pin")[1])*diff(par("usr")[1:2])
post_hist_class_3$counts<-(post_hist_class_3$counts/max_bar)*(par("cex")*1.4*par("cin")[2]/par("pin")[1])*diff(par("usr")[1:2])

#We add the Gardner-Altman part for that panel
add_gardner_altman(TLC_posterior_light_class_1,HC_posterior_light_class_1,difference_posterior_light_class_1,post_hist_class_1)

#Add title
mtext(substitute(paste("Proportion interactions with mounted  ",italic('H. heurippa')," female")),
      2,line=9.9)

#We add the same type of swarm plot, but this time just including data from the heu model under cloudy light class
testout_bayes<-plot_proportion_stats(input_data=pref_stat_TLC_HC[ifelse(is.na(pref_stat_TLC_HC$light_class_HC),F,pref_stat_TLC_HC$light_class_HC=="cloudy"),],
                                     # The data set
                                     sum_by_col_plot=c("ID"),
                                     # Sum by ID for plot
                                     response_col="court_lin_heu_01",
...
#We add the Gardner-Altman part for that panel
add_gardner_altman(TLC_posterior_light_class_2,HC_posterior_light_class_2,difference_posterior_light_class_2,post_hist_class_2)

#We add the same type of swarm plot, but this time just including data from the heu model under woodland shade light class
testout_bayes<-plot_proportion_stats(input_data=pref_stat_TLC_HC[ifelse(is.na(pref_stat_TLC_HC$light_class_HC),F,pref_stat_TLC_HC$light_class_HC=="sunny_but_treeshade"),],
                                     # The data set
                                     sum_by_col_plot=c("ID"),
                                     # Sum by ID for plot
                                     response_col="court_lin_heu_01",
...
#We add the Gardner-Altman part for that panel
add_gardner_altman(TLC_posterior_light_class_3,HC_posterior_light_class_3,difference_posterior_light_class_3,post_hist_class_3)
...
```{r}
post_hist_BC_cat_1<-hist(difference_posterior_BC_cat_1,breaks=seq(floor(200*min(difference_posterior_BC_cat_1))/200,
                                                            ceiling(200*max(difference_posterior_BC_cat_1))/200,0.005),plot=F)
post_hist_BC_cat_2<-hist(difference_posterior_BC_cat_2,breaks=seq(floor(200*min(difference_posterior_BC_cat_2))/200,
                                                            ceiling(200*max(difference_posterior_BC_cat_2))/200,0.005),plot=F)
```
...
#We add the same type of swarm plot, but this time just including data from dark conditions at the heu model
testout_bayes<-plot_proportion_stats(input_data=pref_stat_BC[ifelse(is.na(pref_stat_BC$quantile_low_up_1),F,pref_stat_BC$quantile_low_up_1=="q1"),],
                                     # The data set
                                     sum_by_col_plot=c("ID"),
                                     # Sum by ID for plot
                                     response_col="court_lin_heu_01",
...
post_hist_BC_cat_1$counts<-(post_hist_BC_cat_1$counts/max_bar)*(par("cex")*1.4*par("cin")[2]/par("pin")[1])*diff(par("usr")[1:2])
post_hist_BC_cat_2$counts<-(post_hist_BC_cat_2$counts/max_bar)*(par("cex")*1.4*par("cin")[2]/par("pin")[1])*diff(par("usr")[1:2])

#We add the Gardner-Altman part for that panel
add_gardner_altman(non_red_posterior_cat_1,red_posterior_cat_1,difference_posterior_BC_cat_1,post_hist_BC_cat_1)


#We add the same type of swarm plot, but this time just including data from bright heu conditions
testout_bayes<-plot_proportion_stats(input_data=pref_stat_BC[ifelse(is.na(pref_stat_BC$quantile_low_up_1),F,pref_stat_BC$quantile_low_up_1=="q2"),],
                                     # The data set
                                     sum_by_col_plot=c("ID"),
                                     # Sum by ID for plot
                                     response_col="court_lin_heu_01",
...
#We add the Gardner-Altman part for that panel
add_gardner_altman(non_red_posterior_cat_2,red_posterior_cat_2,difference_posterior_BC_cat_2,post_hist_BC_cat_2)
...
Split posteriors by type and illuminance
```{r}
F1_posterior_cat_1<-other_hyb_posteriors_cat$value[other_hyb_posteriors_cat$type=="F1"&other_hyb_posteriors_cat$quantile_low_up_1=="q1"]
BC_H_posterior_cat_1<-other_hyb_posteriors_cat$value[other_hyb_posteriors_cat$type=="HCx(TLCxHC)"&other_hyb_posteriors_cat$quantile_low_up_1=="q1"]
F1_posterior_cat_2<-other_hyb_posteriors_cat$value[other_hyb_posteriors_cat$type=="F1"&other_hyb_posteriors_cat$quantile_low_up_1=="q2"]
BC_H_posterior_cat_2<-other_hyb_posteriors_cat$value[other_hyb_posteriors_cat$type=="HCx(TLCxHC)"&other_hyb_posteriors_cat$quantile_low_up_1=="q2"]
```
...
#Calling plot_proportion_stats to produce the panel with the full data
testout_bayes<-plot_proportion_stats(input_data=pref_stat_TLC_HC[pref_stat_TLC_HC$type=="TLC",],
                                     # The data set
...
#Calling plot_proportion_stats to produce the panel with the full data
testout_bayes<-plot_proportion_stats(input_data=pref_stat_BC[pref_stat_BC$redYN=="N",],
                                     # The data set
                                     sum_by_col_plot=c("ID","quantile_low_up_1"),
                                     # Sum by ID for plot
                                     response_col="court_lin_heu_01",
...
#Calling plot_proportion_stats to produce the panel with the full data
testout_bayes<-plot_proportion_stats(input_data=pref_stat_BC[pref_stat_BC$redYN=="Y",],
                                     # The data set
                                     sum_by_col_plot=c("ID","quantile_low_up_1"),
                                     # Sum by ID for plot
                                     response_col="court_lin_heu_01",
...
#Calling plot_proportion_stats to produce the panel with the full data
testout_bayes<-plot_proportion_stats(input_data=pref_stat_other_hyb[pref_stat_other_hyb$type=="F1",],
                                     # The data set
                                     sum_by_col_plot=c("ID","quantile_low_up_1"),
                                     # Sum by ID for plot
                                     response_col="court_lin_heu_01",
...
#Calling plot_proportion_stats to produce the panel with the full data
testout_bayes<-plot_proportion_stats(input_data=pref_stat_other_hyb[pref_stat_other_hyb$type=="HCx(TLCxHC)",],
                                     # The data set
                                     sum_by_col_plot=c("ID","quantile_low_up_1"),
                                     # Sum by ID for plot
                                     response_col="court_lin_heu_01",
...
#Calling plot_proportion_stats to produce the panel with the full data
testout_bayes<-plot_proportion_stats(input_data=pref_stat_TLC_HC[pref_stat_TLC_HC$type=="HC",],
                                     # The data set
                                     sum_by_col_plot=c("ID","quantile_low_up_1"),
                                     # Sum by ID for plot
                                     response_col="court_lin_heu_01",
...
pure_interaction_heu<-extract_emm_cri(suppressWarnings(suppressMessages(emmeans(mod1.2, ~type|log_light_HC, transform="response",at=list(log_light_HC=val_to_check)))))
...
val_to_check<-do.call(seq,as.list(c(data_range,dist(data_range)/99)))
backcross_interaction_heu<-extract_emm_cri(suppressWarnings(suppressMessages(emmeans(mod2.2, ~redYN|log_light_HC, transform="response",at=list(log_light_HC=val_to_check)))))

#Repeat for effect of illuminance around lin 'female' from mod2.2
data_range<-range(mod2.2$data$log_light_TLC)
val_to_check<-do.call(seq,as.list(c(data_range,dist(data_range)/99)))
backcross_interaction_lin<-extract_emm_cri(suppressWarnings(suppressMessages(emmeans(mod2.2, ~redYN|log_light_TLC, transform="response",at=list(log_light_TLC=val_to_check)))))
```
